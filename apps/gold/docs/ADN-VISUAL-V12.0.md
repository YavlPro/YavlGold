# ADN Visual V12.0 — YavlGold

**Estado:** ACTIVO — Canon visual actual
**Version:** V12.0
**Base anterior:** ADN Visual V11.0 (archivado como referencia histórica)
**Fecha de canonización:** 2026-06-24
**Cambios principales V12:** Tipografía migrada a Plus Jakarta Sans + Inter.
Nuevos componentes: Velocímetro de Salud Financiera (§20), Header de bienvenida
dorado (§21). borderShimmer canonizado en separadores de Dashboard Agro.
...
4. **Tipografía**: Plus Jakarta Sans (títulos, eyebrows, labels), Inter (cuerpo,
   UI, datos numéricos), Playfair Display (citas). Orbitron y Rajdhani:
   **DEPRECADAS** — prohibidas en código nuevo.
...
### Familias tipográficas canónicas

**Plus Jakarta Sans** — títulos de módulo, eyebrows de sección, nombres de
cultivo, labels de estado, chips de navegación, headers de bienvenida.
Pesos: 500, 600, 700.
Import Google Fonts: `family=Plus+Jakarta+Sans:wght@500;600;700`

**Inter** — cuerpo, descripciones, datos numéricos, badges, botones, texto muted.
Pesos: 400, 500, 600.
Import Google Fonts: `family=Inter:wght@400;500;600`

**Playfair Display** — citas editoriales únicamente. Se mantiene sin cambios.

### Fuentes deprecadas (V12)

- **Orbitron**: Prohibida en código nuevo. No replicar. Si existe en CSS legacy,
  no eliminar sin migración planificada.
- **Rajdhani**: Prohibida en código nuevo. No replicar. Si existe en CSS legacy,
  no eliminar sin migración planificada.

El cambio tipográfico aplica a código nuevo y al Dashboard Agro ya migrado.
El CSS legacy que aún usa Orbitron/Rajdhani se migra gradualmente por módulo —
no en bloque para evitar regresiones visuales.
...
- `agro-dashboard-v11.css` — separadores de sección del Dashboard Agro
  (`.block-head::after`, 3s)
...
## §20 — Componente: Velocímetro de Salud Financiera

Indicador SVG animado que comunica el estado financiero general de una finca.
Componente exclusivo del Dashboard Agro (Bloque 3 — "Cómo va mi finca").

### Estados y mapeo

| Estado display | Condición financiera | Posición aguja | Color |
|---|---|---|---|
| NEUTRAL | cobradoReal < inversionTotal, sin fiados | -60° | `--text-muted` (gris) |
| RECUPERANDO | fiadosPendientes > 0 | 0° | `--color-warning` (ámbar) |
| GANANDO | cobradoReal >= inversionTotal | +60° | `--color-success` (verde) |

### Distinción crítica con estados de cultivo

Los estados del velocímetro (NEUTRAL / RECUPERANDO / GANANDO) son **distintos**
de los estados de ciclo de cultivo del Manifiesto §4.3
(GANADO / RECUPERANDO / INVIRTIENDO / EQUILIBRIO).

- El velocímetro lee la **finca en conjunto** — salud financiera macro.
- Los estados de cultivo leen un **ciclo individual** — rentabilidad micro.
- No mezclar, no confundir, no intercambiar.
...
.gauge-arc { opacity: 0.18; transition: opacity 600ms ease; }
.gauge-arc.active { opacity: 1; }
.gauge-needle {
...
.gauge-status-name.is-success { color: var(--color-success); }
.gauge-status-name.is-warning { color: var(--color-warning); }
```

### Reglas de uso

- Exclusivo del Dashboard Agro, Bloque 3.
- Máximo uno por vista.
- Nunca en modales, cards de cultivo, factureros ni listas.
- La transición de aguja usa `cubic-bezier(0.34, 1.56, 0.64, 1)` — no modificar.
- Respetar `prefers-reduced-motion`: si activo, transición instantánea sin animación.

---

## §21 — Patrón: Header de bienvenida dorado

El H1 del Dashboard Agro ("Bienvenido, [nombre]") usa `--gold-4` en lugar de
`--text-primary`. Es la única excepción canónica donde un título de bienvenida
es completamente dorado.
...
- Solo en el header principal del Dashboard Agro.
- No extender a otros módulos sin autorización explícita.
- Los títulos de sección (`.block-title`) mantienen `--text-primary` (blanco).
  Solo el H1 de bienvenida es completamente dorado.

---

© 2026 YavlGold · ADN Visual V12.0

...
function getSelectedBuyerRow() {
    ...
    const found = sourceRows.find((row) => String(row?.buyer_id || '') === selectedBuyerId) || null;
    if (found) return found;
    // Fallback: if a crop filter is active and the buyer was not found in the
    // scoped rows (e.g. their movements still have the old buyer_group_key after
    // a rename and were not yet re-associated), look them up in the full summary.
    // This prevents the detail view from collapsing to "Cliente no encontrado"
    // when the buyer clearly exists but the crop scope keys are stale.
    if (getSelectedCropId()) {
...
export function initAgroCarteraVivaView() {
    ...
    // Intercept transfer/revert action buttons rendered inside the buyer history
    // detail view. These buttons are generated by agro-facturero-clientes-detail.js
    // and must be handled here (inside the Facturero de Clientes module) so that
    // the full transfer flow runs with the correct context and toast routing.
    // Without this, the clicks fall through to agro.js which uses notifyFacturero
    // targeted at the inline Facturero tabs, not the Cartera Viva view.
    document.addEventListener('click', (event) => {
        ...
        // Only handle clicks that originate inside the Cartera Viva root node.
        if (!root.contains(target)) return;

        const bridge = typeof window !== 'undefined' ? window._agroFactureroBridge : null;

        const revertIncomeBtn = target.closest('.btn-revert-income');
        if (revertIncomeBtn) {
            event.preventDefault();
            event.stopPropagation();
            const itemId = String(revertIncomeBtn.dataset.id || '').trim();
            if (itemId && typeof bridge?.handleRevertIncome === 'function') {
                bridge.handleRevertIncome(itemId);
            }
            return;
        }

        const transferIncomeBtn = target.closest('.btn-transfer-income');
        if (transferIncomeBtn) {
            event.preventDefault();
            event.stopPropagation();
            const itemId = String(transferIncomeBtn.dataset.id || '').trim();
            if (itemId && typeof bridge?.handleIncomeTransfer === 'function') {
                bridge.handleIncomeTransfer(itemId);
            }
            return;
        }

        const revertLossBtn = target.closest('.btn-revert-loss');
        if (revertLossBtn) {
            event.preventDefault();
            event.stopPropagation();
            const itemId = String(revertLossBtn.dataset.id || '').trim();
            if (itemId && typeof bridge?.handleRevertLoss === 'function') {
                bridge.handleRevertLoss(itemId);
            }
            return;
        }

        const transferLossBtn = target.closest('.btn-transfer-loss');
        if (transferLossBtn) {
            event.preventDefault();
            event.stopPropagation();
            const itemId = String(transferLossBtn.dataset.id || '').trim();
            if (itemId && typeof bridge?.handleLossTransfer === 'function') {
                bridge.handleLossTransfer(itemId);
            }
            return;
        }
    }, true); // useCapture=true so this fires before agro.js delegation

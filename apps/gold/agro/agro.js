...
function buildActiveCycleCardsData(crops, options = {}) {
    ...
    return rows.map((crop) => {
        ...
        return {
            ...
            durationDays: null,
            seedKg: seedKgValue,
            inversionUSD: baseInvestment + expenseInvestment,
            rentabilidad: net,
...
async function loadIncomes() {
    ...
    try {
        ...
        const signedUrlMap = await buildIncomeSignedUrlMap(data);
...
// ANEXO 25 S1 (2026-09-17): el bloque completo vive en agro-assistant.js
// (+ agro-assistant-ui.js + agro-assistant.css). Este monolito solo
// expone window._agroAssistantDeps y llama window.initAgroAssistantSurface
// (ver initAgro). randomBase36 se conserva aquí por initAccordions.
// ============================================================
...
export function initAgro() {
    ...
    // ANEXO 25 S1: puente de deps para agro-assistant.js (extracción del
    // monolito). Lectura perezosa: el módulo lo consulta en cada uso.
    window._agroAssistantDeps = {
        getCrops: () => (Array.isArray(cropsCache) && cropsCache.length > 0
            ? cropsCache
    ...
    window.initAgroAssistantSurface?.();

...
function buildCyclesWorkspaceSnapshot(payload = {}) {
    const active = Array.isArray(payload.active) ? payload.active : [];
    const finished = Array.isArray(payload.finished) ? payload.finished : [];
    const lost = Array.isArray(payload.lost) ? payload.lost : [];
    const auditCount = Math.max(0, Number(payload.auditCount || 0) || 0);
    const total = Number.isFinite(Number(payload.total))
        ? Number(payload.total)
        : (active.length + finished.length + lost.length);
    return {
        active,
        finished,
        lost,
        ...
        summary: {
            ...
            lost: lost.length,
            audit: auditCount,
            closed: finished.length + lost.length,
            total
        },
...
function publishCyclesWorkspaceSnapshot(snapshot = null) {
    cyclesWorkspaceSnapshot = snapshot && typeof snapshot === 'object'
        ? snapshot
        : createEmptyCyclesWorkspaceSnapshot();
    if (typeof window === 'undefined') return;
    window._agroCyclesWorkspace = Object.assign(window._agroCyclesWorkspace || {}, {
        getSnapshot: () => cyclesWorkspaceSnapshot
    });
    window.dispatchEvent(new CustomEvent('agro:cycles:snapshot', {
        detail: cyclesWorkspaceSnapshot
    }));
}
...
function createEvidenceLinkElement(url) {
    ...
    link.className = 'facturero-evidence-link tx-evidence';
...
function createFactureroActionButton({
    ...
    const btn = document.createElement('button');
    btn.type = 'button';
    const baseClassName = String(className || '').trim();
    const lowerClassName = baseClassName.toLowerCase();
    let hoverClass = '';
    if (lowerClassName.includes('delete')) {
        hoverClass = 'btn-danger-hover';
    } else if (lowerClassName.includes('revert')) {
        hoverClass = 'btn-success-hover';
    } else if (lowerClassName.includes('transfer') || lowerClassName.includes('move-general')) {
        hoverClass = 'btn-transfer-hover';
    }
    btn.className = [baseClassName, 'btn', 'btn-icon-only', 'btn-ghost', hoverClass].filter(Boolean).join(' ');
    ...
    if (effectiveOpacity !== '1') {
        btn.style.opacity = effectiveOpacity;
    }
...
function renderHistoryRow(tabName, item, config, options = {}) {
    ...
    const normalizedStatusToken = String(
        item?.status
        || item?.estado
        || item?.estado_texto
        || item?.state
    ...
    const isFiadoSettledByStatus = /(pagado|cobrado|liquidado)/.test(normalizedStatusToken);

    let txTypeClass = 'tx-gasto';
    if (isPending) {
        txTypeClass = isFiadoSettledByStatus ? 'tx-pagado' : 'tx-pendiente';
    } else if (effectiveTabName === 'gastos') {
        txTypeClass = 'tx-gasto';
    } else if (isLoss) {
        txTypeClass = 'tx-perdida';
    } else if (effectiveTabName === 'transferencias' || effectiveTabName === 'donaciones') {
        txTypeClass = 'tx-donacion';
    } else if (effectiveTabName === 'otros') {
        txTypeClass = 'tx-otro';
    } else if (isIncome) {
    ...
    const hasTransferStateMeta = transferStateToken === 'transferred'
        ...
        || transferStateToken === 'reverted_to_pending';
    const hasTransferMeta = transferred
        || pendingReverted
        || fromPending
        || incomeOrLossReverted
    ...
    } else if (transferred || fromPending || item?.other_transfer_state === 'transferred') {
        statusVariant = 'transferido';
        statusText = 'Transferido';
    } else if (isPending && isFiadoSettledByStatus) {
        statusVariant = 'pagado';
        statusText = 'Pagado';
    } else if (isLoss) {
        statusVariant = 'perdida';
    ...
    if (fromPending) {
        ...
        originBadge.className = 'facturero-origin-badge tx-detail';
        ...
        meta.className = 'facturero-meta facturero-transfer-meta facturero-origin-meta tx-detail';
    ...
    const amountMain = amountParts.shift() || formattedAmount || '$0';
...
function renderHistoryRowFallback(item, config) {
    ...
    amountWrap.className = 'tx-amount-group';
...
function buildNoActiveCropsCard(finishedCount = 0) {
    return buildCropsStatusCard({
        ...
        subtitle: finishedCount > 0
            ? `Revisa Ciclos finalizados (${finishedCount}) para ver ciclos cerrados`
...
function createCycleHistoryGroupSection(config = {}) {
    ...
    title.textContent = String(config.titleText || 'Ciclos finalizados');
...
export async function loadCrops() {
    const cropsGrid = document.getElementById('cyclesContainer');
    if (!cropsGrid) return;
    ...
    try {
        ...
        const { finished: visibleClosedFinishedCrops, lost: visibleLostCrops } = splitClosedCycleHistory(visibleFinishedCrops);
        const finishedCycleCards = buildFinishedCycleCardsData(visibleClosedFinishedCrops, {
            expenseTotalsByCrop,
            incomeTotalsByCrop,
            lossTotalsByCrop,
            pendingTotalsByCrop,
...
function renderOpsCultivosPanel() {
    ...
    historyButton.setAttribute(
        ...
        finishedCrops.length > 0
            ? `Ir a ciclos finalizados (${finishedCrops.length})`
            : 'Ir a ciclos finalizados'
    );

    if (finishedNote) {
        finishedNote.textContent = finishedCrops.length > 0
            ? `${finishedCrops.length} ciclos cerrados viven en Ciclos finalizados.`
            : 'Finalizados y perdidos viven en Ciclos finalizados.';
    }
}
...
function matchesPagadosDedicatedStateFilter(item, filter = pagadosDedicatedStateFilter) {
    ...
    if (token === 'all') {
        return getPagadosDedicatedStateToken(item) !== 'reverted';
    }
...
function matchesFiadosDedicatedFilters(pending, options = {}) {
    ...
    return fiadosDedicatedPassesToggle(pending, options);
}
...
function getFiadosDedicatedViewState() {
    ...
    const allMatchingRows = getFiadosDedicatedRows(sourceRows, { cropId, searchQuery, skipToggle: true });
    const transferredCount = allMatchingRows.filter((item) => isPendingTransferred(item)).length;
    const revertedCount = allMatchingRows.filter((item) => isPendingReverted(item)).length;
    const activeCount = allMatchingRows.length - transferredCount - revertedCount;

    const rows = getFiadosDedicatedRows(sourceRows, {
        cropId,
        searchQuery,
        showTransferred: fiadosDedicatedShowTransferred,
        showReverted: fiadosDedicatedShowReverted
    });

    const hasActiveFilters = fiadosDedicatedShowTransferred || fiadosDedicatedShowReverted;
    const hasDynamicFilters = transferredCount > 0 || revertedCount > 0;
...
async function renderFiadosDedicatedView() {
    ...
    if (status) {
        if (context.hasSearch && filtered.length === 0) {
            ...
            status.textContent = context.showTransferred || context.showReverted
                ? `Fiados activos${context.showTransferred ? ' + transferidos' : ''}${context.showReverted ? ' + revertidos' : ''} en Vista general.`
                : 'Fiados activos en Vista general. Transferidos y revertidos ocultos.';
        } else {
            status.textContent = context.showTransferred || context.showReverted
                ? `Fiados activos${context.showTransferred ? ' + transferidos' : ''}${context.showReverted ? ' + revertidos' : ''} de ${context.scopeLabel}.`
                : `Fiados activos de ${context.scopeLabel}. Transferidos y revertidos ocultos.`;
        }
    }
    ...
    if (emptyText) {
        emptyText.textContent = context.hasSearch
            ...
            : `No hay fiados activos en ${context.scopeLabel}.`;
    }
...
function bindFiadosDedicatedView() {
    ...
    filterBar?.addEventListener('click', (event) => {
        ...
        if (!filterButton || filterButton.hidden) return;
        const filterType = filterButton.dataset.fiadosFilter;
        if (filterType === 'transferred') {
            fiadosDedicatedShowTransferred = !fiadosDedicatedShowTransferred;
        } else if (filterType === 'reverted') {
            fiadosDedicatedShowReverted = !fiadosDedicatedShowReverted;
        }
        renderFiadosDedicatedView();
    });
    ...
    document.addEventListener('agro:pending:refreshed', renderFiadosDedicatedView);
}
...
window.openAgroAgenda = async function (opts) {
    try {
        ...
        syncLazyCropConsumers(cropsCache);
        await initAgroAgenda({ supabase, cropsCache, inline: opts?.inline === true });
...
// ============================================================
// AGROREPO — lazy-loaded from agrorepo.js
// ============================================================
(async function loadAgroRepo() {
    try {
        const { initAgroRepo } = await import('./agrorepo.js');
        initAgroRepo();
    } catch (err) {
        console.error('[AgroRepo] Failed to load module:', err);
    }
})();

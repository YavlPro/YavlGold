-- REPAIR MIGRATION - YavlGold Agro buyer portfolio RPC current contract restore
-- Created on: 2026-04-18
--
-- Purpose:
-- Restore the current public.agro_buyer_portfolio_summary_v1() contract after
-- older short-contract patches have applied during a clean root bootstrap.
--
-- Reason:
-- 20260330173000 introduced the client-master fields canonical_name and
-- client_status in the RPC return shape. Later migrations 20260331000000 and
-- 20260417113444 rewrote the same RPC with the older short return shape. The
-- order repair before 20260331000000 lets those legacy patches run; this final
-- forward repair restores the current long return shape and keeps the latest
-- transfer-marker logic from 20260417113444.
--
-- This migration does not edit older migrations and does not use
-- apps/gold/supabase/ as canon.
...
'Agregacion client-centric de Cartera Viva V1. Restaura canonical_name/client_status y respeta marcadores de transferencia legacy.';

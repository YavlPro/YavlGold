-- Fix: agro_rank_top_clients - column reference "created_at" is ambiguous
-- Problema: MAX(created_at) y WHERE created_at son ambigüos cuando JOIN con agro_buyers
-- Solución: qualificar todos los refs de columna con i. en el template SQL
...
BEGIN
    ...
    -- All column refs qualified with i. to avoid ambiguity with JOIN agro_buyers
    v_sql := format('
        SELECT
            %1$s AS buyer_name,

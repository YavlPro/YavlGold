-- INTEGRITY FIX — YavlGold Agro buyer portfolio RPC
-- Created: 2026-06-06
--
-- Problem:
-- agro_buyer_portfolio_summary_v1() includes movements (pending, income, losses)
-- that belong to soft-deleted crops (agro_crops.deleted_at IS NOT NULL).
-- This causes ghost buyers like "David" and "Reyes" (linked to deleted "Pepino"
-- crop) to appear in Facturero de Clientes with active pending balances.
--
-- Fix:
-- Add a CTE `valid_crop_ids` that collects non-deleted crop IDs for the session
-- user. All movement CTEs (pending_matched_source, review_union, non_debt_income)
-- now join against this set, dropping orphaned rows silently.
-- Movements without a crop_id (null crop_id = general/farm movements) are kept.
--
-- Surfaces fixed: Facturero de Clientes list, portfolio summary totals.
-- Surfaces already fixed (JS layer): Rankings, section-stats, fetchOperationalProgressMap.
--
-- Canon: MANIFIESTO_AGRO §7 — honestidad documental, cero registros fantasma.
...
as $$
    ...
    -- Valid (non-deleted) crop IDs for the session user.
    -- Movements referencing a crop outside this set are orphaned and excluded.
    -- Null crop_id rows (general/farm movements) pass through.
    valid_crop_ids as (
        select c.id as crop_id
        from public.agro_crops c
        join scope s on s.user_id = c.user_id
    ...
    pending_matched_source as (
        select
            p.buyer_id,
            coalesce(p.monto_usd, p.monto, 0)::numeric as amount,
            lower(coalesce(p.transfer_state, '')) as transfer_state,
            lower(trim(coalesce(p.transferred_to, ''))) as transferred_to,
            p.transferred_income_id
        from public.agro_pending p
        join scope s
          on s.user_id = p.user_id
        -- Exclude movements from deleted crops; keep null-crop (general) movements.
        left join valid_crop_ids vc on vc.crop_id = p.crop_id
        where p.deleted_at is null
          and p.reverted_at is null
          and p.buyer_id is not null
          and p.buyer_match_status = 'matched'
          and (p.crop_id is null or vc.crop_id is not null)
    ),
    pending_matched as (
        select
            buyer_id,
            sum(amount)::numeric as credited_total,
            sum(
                case
                    when transfer_state = 'transferred'
                      or transferred_income_id is not null
                      or transferred_to <> ''
                    then 0
                    else amount
                end
            ...
            sum(
                case
                    ...
                    and transferred_income_id is null
                    and transferred_to not in ('income', 'losses')
                    then amount
                    else 0
                end
            )::numeric as transferred_total
        from pending_matched_source
        group by buyer_id
    ),
    income_paid as (
        select
        ...
        left join valid_crop_ids vc on vc.crop_id = i.crop_id
        where i.deleted_at is null
          ...
          and (i.crop_id is null or vc.crop_id is not null)
        group by i.buyer_id
    ),
    loss_matched as (
        select
        ...
        left join valid_crop_ids vc on vc.crop_id = l.crop_id
        where l.deleted_at is null
          ...
          and (l.crop_id is null or vc.crop_id is not null)
        group by l.buyer_id
    ),
    review_union as (
        select
...
'Agregacion client-centric de Cartera Viva V1. Excluye movimientos de cultivos con deleted_at IS NOT NULL (valid_crop_ids CTE). Restaura canonical_name/client_status y respeta marcadores de transferencia legacy.';

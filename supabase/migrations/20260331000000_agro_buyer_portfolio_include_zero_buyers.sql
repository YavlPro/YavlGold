...
-- BUGFIX: agro_buyer_portfolio_summary_v1 — include brand-new buyers (all metrics = 0)
-- so they appear in Cartera Viva grid and can receive their first movement.
--
-- Previously the RPC WHERE clause required at least one metric > 0, which excluded
-- brand-new buyers with no history. This caused "Cliente no encontrado" when
-- creating a new buyer and immediately opening their detail view.
--
-- NO breaking change: the WHERE clause only adds a new condition for zero-value rows.

create or replace function public.agro_buyer_portfolio_summary_v1()
returns table(
    buyer_id uuid,
    display_name text,
    group_key text,
...
as $$
    ...
    from portfolio p
    where p.credited_total > 0
       or p.paid_total > 0
       or p.loss_total > 0
       or p.pending_total > 0
       or p.review_required_total > 0
       or p.legacy_unclassified_total > 0
       -- BUGFIX: include brand-new buyers (all metrics = 0) so they appear
       -- in Cartera Viva grid and can receive their first movement.
       or (
           p.credited_total = 0
           and p.paid_total = 0
           and p.loss_total = 0
    ...
    order by p.pending_total desc nulls last, p.review_required_total desc nulls last, p.display_name asc;
$$;

comment on function public.agro_buyer_portfolio_summary_v1() is
'Agregacion buyer-centric de Cartera Viva V1. Incluye clientes nuevos sin movimientos (all metrics = 0).';

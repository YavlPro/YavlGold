-- Fix: agro_rank_top_clients - usar display_name en JOIN con agro_buyers
-- La columna se llama display_name, no name
...
BEGIN
    ...
    -- Buyer name resolution: dedicated column > buyer_id JOIN > concepto regex
    ...
    ELSIF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'agro_income' AND column_name = 'buyer_id') AND EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'agro_buyers') THEN
        v_has_buyer_id := true;

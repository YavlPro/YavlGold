-- Migración: Fix agro_rank_top_clients para extraer nombre desde concepto
-- Fecha: 2026-06-18
-- Problema: agro_income no tiene columnas de nombre de comprador (comprador, cliente, buyer_name, etc.)
--           La RPC retornaba 'Sin nombre' para todas las filas, vaciando el ranking de clientes.
-- Solución: JOIN con agro_buyers para obtener el nombre, fallback a regex sobre campo concepto.
...
BEGIN
    ...
    -- Detect buyer name source (priority: dedicated column > buyer_id JOIN > concepto regex)
    IF EXISTS (
        SELECT 1 FROM information_schema.columns

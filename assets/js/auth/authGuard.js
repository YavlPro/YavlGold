...
console.log('[AuthGuard] ✅ AuthGuard v2.0.1 cargado');
    ...
    console.log('[AuthGuard] ➡ Redirigiendo a dashboard (default)');
    setTimeout(() => (window.location.href = '/dashboard/'), 800);
  },
  ...
  checkRole(requiredRole) {
    if (!this.check()) return false;
    if (!this.hasRole(requiredRole)) {
      console.warn(`[AuthGuard] ⛔ Rol insuficiente. Requiere: ${requiredRole}`);
      if (window.AuthUI) {
        window.AuthUI.showError('generic', 'No tienes permisos para acceder a esta página');
      } else {
  ...
  protectByRole() {
    ...
    document.querySelectorAll('[data-role]').forEach((el) => {
      ...
      if (!this.hasRole(required)) {
        ...
        console.log(`[AuthGuard] 🔒 Elemento oculto (rol requerido: ${required})`);
      } else {
        el.style.display = '';
      }
    });
  ...
  init() {
    ...
    // Redirección tras login y actualización de protecciones
    window.addEventListener('auth:login', () => {
      console.log('[AuthGuard] 🔓 Login detectado');
      this.protectLinks();
      this.protectByRole();
      this.redirectAfterLogin();
    });

    // En logout, si estás en una ruta protegida, volver a home
    window.addEventListener('auth:logout', () => {
      console.log('[AuthGuard] 🔒 Logout detectado');
      this.protectLinks();
      if (this.isProtectedRoute()) setTimeout(() => (window.location.href = '/'), 500);
    });
  },
};
